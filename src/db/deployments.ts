...
export const DEPLOYMENTS: DeploymentInsert[] = [
  ...
  },
  {
    ...
    thinking: "n/a",
    params:
      "api_mode chat_completions; context_length 850000; NO reasoning_effort set (falls through to provider default) — the transport patch strips it for gpt-5.x only when tools are present, and Hermes always sends tools",
    config_ref: "hermes-agent/config.yaml:36-47",
    rationale:
      "Structural: a different vendor than the brain (gpt-5.x vs DeepSeek), same IU OpenAI leg, cached — chosen so a brain outage doesn't take out the whole path. Moved off the EU-pinned claude-sonnet-4-6-eu on the 2026-09-13 rollout, dropping the region guarantee that pick carried — see hermes-brain.md.",
    decision_doc: "docs/decisions/hermes-brain.md",
    decided_at: "2026-09-13",
    verified_at: VERIFIED,
  },
  ...
  },
  {
    ...
    thinking: "low",
    params:
      "api_mode chat_completions; reasoning_effort low; context_length 850000; title_generator.py:268 hardcodes temperature=0.3, stripped for gpt-5.x on this leg by patches/auxiliary-client-iu-openai-leg-quirks.patch",
    config_ref: "hermes-agent/config.yaml:247-260",
    rationale:
      "Moved off gemini-2.5-flash-lite in the 2026-09-13 rollout — low effort keeps titling fast, follows the estate's small-latency-critical-call tier (fast-model.md).",
  ...
  },
  {
    ...
    thinking: "default",
    params:
      "used by gemini-tts.ts:137 and replicate-tts.ts:122; prep runs only for allowlisted models (elevenlabs/v3)",
    config_ref: "audio-gateway/src/config.ts:207,284",
    rationale:
      "The prep pass is why Gemini TTS measured ~10s per reply end to end — it is counted against the TTS latency, not hidden behind it.",
    decision_doc: "docs/decisions/audio-stack.md",
    decided_at: "2026-08-26",
    verified_at: VERIFIED,
  },
  ...
  },
  {
    ...
    category: null,
    thinking: "n/a",
    params: "from the generated playbook (shared/src/playbook.generated.ts:20)",
    config_ref: "image-gen/shared/src/playbook.generated.ts:20",
    rationale:
      "Manual category: sweeps LMArena Image Arena (+242 ELO text-to-image), #1 single/multi-image edit, near-perfect in-image text.",
    decision_doc: "docs/decisions/vision-and-image.md",
    decided_at: "2026-06-17",
    verified_at: VERIFIED,
  },
