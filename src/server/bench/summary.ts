...
// Re-exported so every existing server-side caller (scripts/cap.ts, tests)
// keeps working unchanged — `format.ts` is the new home, this is just the
// door. `/bench` imports these directly from `./format` instead, because a
// value import from *this* module drags `db` along (see format.ts's header).
export {
  aaCodingOf,
  aaIntelligenceOf,
  caveatLines,
...
// ── external index ───────────────────────────────────────────────────────────
// `AaIndex` itself lives in `./format.ts` — it is a pure shape the presentation
// layer also needs, re-exported above.
...
// `aaIntelligenceOf` / `aaCodingOf` live in `./format.ts` — re-exported above,
// imported here for use in the picks/caveats derivation below.
...
// ── rate card ────────────────────────────────────────────────────────────────
// `RateCard` itself lives in `./format.ts` — re-exported above.
...
// `BenchCaveats` itself lives in `./format.ts` — re-exported above.
...
// `caveatLines` and every `format*` helper (+ `MISSING`) live in `./format.ts`
// — re-exported above so `scripts/cap.ts` and existing callers see them
// unchanged.
